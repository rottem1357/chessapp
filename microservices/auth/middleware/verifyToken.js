// microservices/auth/middleware/verifyToken.js
...
const verifyToken = (req, res, next) => {
  try {
    // Get token from Authorization header or X-User-ID header (from gateway)
    const authHeader = req.headers.authorization;
    const userIdHeader = req.headers['x-user-id'];
    const userRoleHeader = req.headers['x-user-role'];
    
    // If gateway already validated token and passed user info
    if (userIdHeader && userRoleHeader) {
      req.user = {
        id: userIdHeader,
        role: userRoleHeader,
        is_admin: userRoleHeader === 'admin'
      };
      return next();
    }
